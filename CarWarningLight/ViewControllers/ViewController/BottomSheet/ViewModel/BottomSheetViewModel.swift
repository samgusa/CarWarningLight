...
final class BottomSheetViewModel {
  ...
  func fetchInfo() -> Future<[InfoData], Never> {
    return Future { promise in
      let sections = Bundle.main.decode([InfoData].self, from: "car.json")
      promise(.success(sections))
    }
  }

  func infoFilter(arr: [InfoData], info: InfoEnum) -> String {
    return arr.filter { $0.name == info.infoName }
      .map { $0.data }.joined()
  }
